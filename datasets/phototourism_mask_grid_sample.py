...
#from utils import get_NDC_rays
class PhototourismDataset(Dataset):
    def __init__(self, root_dir, split='train', img_downscale=1, val_num=1, use_cache=False, batch_size=1024, scale_anneal=-1, min_scale=0.25, masks_dir="./gate_masks"):
        """
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            if self.use_cache:
                ...
                for id_ in self.img_ids_train:
                    ...
                    mask = Image.open(os.path.join(self.masks_dir,self.image_paths[id_])).convert('L')
                    mask_np = np.array(mask) / 255
                    # print("mask_max_value: ", mask_np.max())
                    ...
                    img_mask = mask_tensor.view(1, -1).permute(1, 0) # (h*w, 1) mask
                    ...
                    if self.ndc_rays:
                        n, f = 0, 1
                        rays_o, rays_d = get_NDC_rays(img_h, img_w, self.Ks[id_][0,0], 1.0, rays_o,rays_d)
                    """
                ...
                self.all_imgs_wh = torch.cat(self.all_imgs_wh, 0) # ((N_images-1)*h*w, 2)
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train': # use data in the buffers
            ...
            #z = self.all_imgs_wh[:sample_ts, 0]*self.all_imgs_wh[:sample_ts, 1]
            #print("********",z,z.shape)
            rgb_sample_points = (img_sample_points + (self.all_imgs_wh[:sample_ts, 0]*self.all_imgs_wh[:sample_ts, 1]).sum()).long()
            mask_sample_points = rgb_sample_points
            #print("***************",img_sample_points,img_sample_points.shape)
            #print("-------------",rgb_sample_points,rgb_sample_points.shape)
            sample = {'rays': self.all_rays[rgb_sample_points, :8],
                      ...
                      'uv_sample': uv_sample,
                      'mask': self.all_masks[mask_sample_points]
                      }
            #print("-----------------",sample)
        ...
        return sample
#  python eval.py --root_dir ./brandenburg_gate/ --save_dir save --dataset_name phototourism --scene_name HaNeRF_Brandenburg_Gate --split test_test --img_downscale 2 --N_samples 256 --N_importance 256 --N_vocab 1500 --ckpt_path ./ckpts/phototourism/brandenburg_exp_step_factor/epoch=7.ckpt --chunk 16384 --img_wh 320 240